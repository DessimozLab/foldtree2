#!/bin/bash
#SBATCH --job-name=ft2-se3-dot-mn
#SBATCH --nodes=2
...
# Multi-node SLURM launcher following the Lightning DDP pattern:
# one srun task per GPU, with Lightning managing the distributed sampler.
...
CHECKPOINT_DIR=${CHECKPOINT_DIR:-/capstor/store/cscs/swissai/prep01/foldtree2/results/geometry/se3_dot_multinode_${SLURM_JOB_ID}}

# Make rendezvous and NCCL diagnostics explicit for multi-node startup.
...
# Uncomment or override this on Alps if the node-local network requires it.
...
echo "Launching multi-node frozen-encoder geometry-dot-contact SE3 training"
echo "SLURM: nodes=${SLURM_JOB_NUM_NODES:-unknown}, tasks=${SLURM_NTASKS:-unknown}, node_list=${SLURM_JOB_NODELIST:-unknown}"
...
srun --ntasks-per-node="${SLURM_NTASKS_PER_NODE:-4}" "${CMD[@]}"

echo "Completed multi-node frozen-encoder geometry-dot-contact SE3 training"
