...
class StableChebConv(MessagePassing):
    ...
    def __init__(self,
        ...
        self.in_lin  = Linear(in_channels, hidden_channels, bias=False,
                              weight_initializer='glorot') if in_channels != hidden_channels else nn.Identity()
        self.out_lin = Linear(hidden_channels, out_channels, bias=False,
                              weight_initializer='glorot') if hidden_channels != out_channels else nn.Identity()

        # Raw weight matrices
        self.M = nn.ParameterList([
            nn.Parameter(torch.empty(hidden_channels, hidden_channels))
            for _ in range(K)
        ])
    ...
    def reset_parameters(self):
        ...
        if isinstance(self.in_lin, Linear):
    ...
    @torch.no_grad()
    def _norm(self, edge_index, num_nodes, edge_weight=None, lambda_max=None, dtype=None, batch=None):
        edge_index, edge_weight = get_laplacian(edge_index, edge_weight, self.normalization, dtype, num_nodes)
    ...
    def forward(self, x: Tensor, edge_index: Tensor,
        ...
        x_h = self.in_lin(x)
        edge_index, norm = self._norm(edge_index, x_h.size(0), edge_weight, lambda_max,
                                      dtype=x_h.dtype, batch=batch)
