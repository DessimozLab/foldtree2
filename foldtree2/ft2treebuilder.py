...
class treebuilder():
	...
	def run_raxml_ng(self, fasta_file, matrix_file, nsymbols, output_prefix , iterations = 10 , cores = 8):
		...
		raxml_cmd = raxmlng_path  + ' --model MULTI'+str(self.nchars)+'_GTR{'+matrix_file+'} --redo  --all --bs-trees '+str(iterations)+' --seed 12345 --threads auto{' + str(4*self.ncores) + '} --workers auto --msa '+fasta_file+' --prefix '+output_prefix  + ' --force perf_threads'
